...
class Key {
    constructor() {
        ...
        this.generateNewKeyAndUpdateInDb = () =>
            __awaiter(this, void 0, void 0, function*() {
                const mongoDriver = yield mongoNew_1.MongoDriver.getDriver();
                while (true) {
                    // this is a start of a transaction in MongoDB
                    let keyFromDB = yield mongoDriver.getKeyValueFromKeyName(REFRESH_TOKEN_KEY_NAME_IN_DB);
                    let key;
                    if (keyFromDB === undefined) {
                        let keyValue = yield utils_1.generateNewSigningKey();
                        key = keyValue;
                        let success = yield mongoDriver.insertKeyValueForKeyName(
