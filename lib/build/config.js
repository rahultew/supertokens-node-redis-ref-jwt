...
const validateAndNormalise = config => {
    ...
    const url = utils_1.sanitizeStringInput(mongoInputConfig.url);
    /**
     * following part is deprecated since v3.1.0
     */
    let extraParams = {};
    if (url === undefined) {
        const host = utils_1.sanitizeStringInput(mongoInputConfig.host);
        const port = utils_1.sanitizeNumberInput(mongoInputConfig.port);
        const user = utils_1.sanitizeStringInput(mongoInputConfig.user);
        if (user === undefined) {
            throw error_1.generateError(
                error_1.AuthError.GENERAL_ERROR,
                new Error("mongo config error. url not passed"),
                false
            );
        }
        const password = utils_1.sanitizeStringInput(mongoInputConfig.password);
        ...
        const connectionLimit = utils_1.sanitizeNumberInput(mongoInputConfig.connectionLimit);
        const database = utils_1.sanitizeStringInput(mongoInputConfig.database);
    ...
    if (collectionsMongoInputConfig !== undefined) {
        const signingKey = utils_1.sanitizeStringInput(collectionsMongoInputConfig.signingKey);
        const refreshTokens = utils_1.sanitizeStringInput(collectionsMongoInputConfig.refreshTokens);
        collections = {
            signingKey,
            refreshTokens
        };
    }
    const mongo = Object.assign({ url }, extraParams, { collections });
...
const setDefaults = config => {
    ...
    return {
        mongo: {
            host: config.mongo.host || defaultConfig.mongo.host,
            port: config.mongo.port || defaultConfig.mongo.port,
            user: config.mongo.user,
            password: config.mongo.password,
            connectTimeout: config.mongo.connectTimeout || defaultConfig.mongo.connectTimeout,
            socketTimeout: config.mongo.socketTimeout || defaultConfig.mongo.socketTimeout,
