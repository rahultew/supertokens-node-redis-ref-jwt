...
import { getKeyValueFromKeyName, insertKeyValueForKeyName, updateKeyValueForKeyName } from "./helpers/dbQueriesOld";
...
/**
 * @description called during library init. Should be called after initing Config and Mongo.
...
class SigningKey {
    ...
    private maybeGenerateNewKeyAndUpdateInDb = async (): Promise<{
        ...
        while (true) {
            // this is a start of a transaction in MongoDB
            const mongoDriver = MongoDriver.getDriver();
            let keyFromDB = await mongoDriver.getKeyValueFromKeyName(ACCESS_TOKEN_SIGNING_KEY_NAME_IN_DB);
            let generateNewKey = false;
            if (keyFromDB !== undefined) {
                ...
                if (this.dynamic && Date.now() > keyFromDB.createdAtTime + this.updateInterval) {
                    generateNewKey = true;
                }
            }
            let key = undefined;
            if (keyFromDB === undefined || generateNewKey) {
                let keyValue = await generateNewSigningKey();
                const lastUpdatedSign = keyFromDB === undefined ? undefined : keyFromDB.lastUpdatedSign;
                key = {
                    keyValue,
                    createdAtTime: Date.now()
                };
                let success;
                if (lastUpdatedSign === undefined) {
                    // no row exists in db
                    success = await mongoDriver.insertKeyValueForKeyName(
                    ...
                    success =
                        (await mongoDriver.updateKeyValueForKeyName(
                            ACCESS_TOKEN_SIGNING_KEY_NAME_IN_DB,
                            key.keyValue,
                            key.createdAtTime,
                            lastUpdatedSign
                ...
                if (!success) {
                    // something already inserted or updated into mongo, so we start again.
                    continue;
                }
            } else {
                key = {
                    keyValue: keyFromDB.keyValue,
                    createdAtTime: keyFromDB.createdAtTime
                };
            }
            return key;
        }
