...
import { getKeyValueFromKeyName, insertKeyValueForKeyName } from "./helpers/dbQueriesOld";
...
class Key {
    ...
    private generateNewKeyAndUpdateInDb = async (): Promise<string> => {
        ...
        while (true) {
            ...
            let keyFromDB = await mongoDriver.getKeyValueFromKeyName(REFRESH_TOKEN_KEY_NAME_IN_DB);
            let key;
            if (keyFromDB === undefined) {
                let keyValue = await generateNewSigningKey();
                key = keyValue;
                let success = await mongoDriver.insertKeyValueForKeyName(REFRESH_TOKEN_KEY_NAME_IN_DB, key, Date.now());
                if (!success) {
                    // something already inserted into mongo since we read. So we try again
                    continue;
                }
            } else {
                key = keyFromDB.keyValue;
            }
            return key;
        }
    };
